...
// Define la función de cancelación
const cancelAppointment = async (id) => {
    try {
        await appointments.cancelAppointment(id); // Suponiendo que este método retorna una promesa
        // Aquí podrías manejar un mensaje de éxito o error si es necesario
    } catch (error) {
        console.error('Error al cancelar la cita:', error);
        // Aquí podrías mostrar un mensaje al usuario si la cancelación falla
    }
};
</script>
    ...
    <div class="bg-white p-5 space-y-3 rounded-lg">
        ...
        <div class="flex gap-2 items-center">
            ...
            <button
                ...
                @click="cancelAppointment(appointment._id)" <!-- Cambiado para usar la función -->
            >
                Cancelar Cita
            </button>
        </div>
